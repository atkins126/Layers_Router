...
type
  ...
  public
    constructor Create(ARttiMethod: TRttiMethod; AEventType: TClass;
      AThreadMode: TThread_Mode; const AContext: String = '';
      APriority: Integer = 1);
  ...
  TSubscribers_Finder = class(TObject)
...
constructor TSubscriber_Method.Create(ARttiMethod: TRttiMethod;
  AEventType: TClass; AThreadMode: TThread_Mode; const AContext: String = '';
  APriority: Integer = 1);
begin
  FMethod := ARttiMethod;
...
class function TSubscribers_Finder.FindSubscriberMethods(ASubscriberClass
  : TClass; ARaiseExcIfEmpty: Boolean = False): TArray<TSubscriber_Method>;
var
  LRttiType: TRttiType;
...
begin
  ...
  for LMethod in LRttiMethods do
    if TRTTIUtils.HasAttribute<Subscribe_Attributes>(LMethod, LSubscribeAttribute)
    then
    begin
