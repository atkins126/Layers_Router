...
{$IFEND}
    and ((LCTX.GetType(AObject.ClassInfo).GetMethod('GetItem') <> nil) or (LCTX.GetType(AObject.ClassInfo).GetMethod('GetElement')<>
    nil)) and (LCTX.GetType(AObject.ClassInfo).GetProperty('Count') <> nil)
...
{$IFEND}
  ...
  begin
    ...
    if not Assigned(FGetCountMethod) then
      ...
      raise EJKException.Create('Cannot find property/method "Count" in the duck object');
  end;
end;
